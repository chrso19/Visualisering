...
At the top of the dashboard, there is a time period selector and filter for plant categories, which can be seen in Figure~\ref{fig:Filter}. 
The plant filter includes a dropdown menu for each plant category, where it is possible to select specific plant species. 
...
As one's cursor moves across the map, a hover box appears containing the observations by category in the selected hexagon and the total number of observations.
When a hexagon is selected, a red circle appears to highlight the area selected.  

Once a hexagon has been selected, a mini map is generated to show the area in greater detail, as shown in Figure~\ref{fig:MiniMap}. 
...
It is possible to go back to the previous histogram.

The purpose of the dashboard is to be simple and easy to use, regardless of one's proficiency with technology. 
It allows for a quick overview of where plants are observed in Denmark, and which categories these observations belong to. 
...
Finally, the colors were tested to ensure that they would also be differentiable by people with colorblindness. 
